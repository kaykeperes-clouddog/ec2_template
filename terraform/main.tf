# Cria um Security Group simples para permitir SSH ja morant seth curry
resource "aws_security_group" "allow_ssh" {
  name        = "allow-teste"
  description = "Permite acesso SSH (porta 22) de qualquer lugar"
  # Usa a VPC Padrão se 'vpc_id' não for especificado agora vai pelo amor gianniss

  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # ATENÇÃO: Permite SSH de qualquer IPv4 Kobe Bryant
  }
